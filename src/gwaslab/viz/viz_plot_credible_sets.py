...
def _plot_cs(pipcs_raw,
        ...
        if region is not None:
                ...
                log.write(f" -Loading PIP and CS for variants in the region :{region}")

        if locus is not None:
                pipcs = pipcs.loc[pipcs["LOCUS"] == locus,:].copy()
                log.write(f" -Loading PIP and CS for variants in the locus :{region}")
                if region is None:
                        region = (pipcs[chrom].iloc[0],pipcs[pos].min(),pipcs[pos].max())
                        log.write(f" -Extracted region:{region}")
        if onlycs ==True:
        ...
        for i,label in enumerate(labels):
                if label in [str(j) for j in range(1,10)]:
                        if cs_category in pipcs.columns:
                                new_labels.append(f"#{label} - {cs_category_dic[cs_category][label]}" )
                        else:
