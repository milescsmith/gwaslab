...
def plot_track(
    ...
    color: str = "#020080",
    highlight_positions: list[float] | None = None,
    ...
    if len(subfeatures_df) > 0:
        ...
        for index, row in subfeatures_df.iterrows():
            ...
            for highlighted_name, highlighted_left, highlighted_right in zip(
                highlighted_names, highlighted_lefts, highlighted_rights, strict=False
            ):
...
def _process_bed_track(
    ...
    # Load BED file
    log.write("  -Loading BED file", verbose=verbose)
...
def _process_bigbed_track(
    ...
    elif "rest" in bb_region.columns:
