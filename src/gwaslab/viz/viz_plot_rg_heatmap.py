...
def plot_rg(ldscrg,
    ...
    log.write(f"Full cell : {full_cell[0]}-corrected P == {full_cell[1]}" ,verbose=verbose)
    ...
    if rganno is not False:
        ...
        for i in rgtoanno:
            ...
            if "color" in rganno_default_kwargs.keys() or "c" in rganno_default_kwargs.keys():
                ax.text(i[0],i[1],f"{i[2]:.3f}",**rganno_default_kwargs)
            else:
                ax.text(i[0],i[1],f"{i[2]:.3f}",color=i[3],**rganno_default_kwargs)
    ...
    if panno is True:
        ...
        for i,correction in enumerate(corrections):
            for j,sig_level in enumerate(sig_levels):
                index = len(sig_levels)*i + j
                log.write(f" -{panno_texts[index]} : {correction}-corrected P < {sig_level} " ,verbose=verbose)
