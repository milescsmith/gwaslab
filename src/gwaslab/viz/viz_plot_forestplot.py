...
try:
    from statsmodels.stats.meta_analysis import combine_effects, effectsize_smd
...
def plot_forest(
    ...
    study_col: str,
    group_col: Union[str, bool] | None = None,
    beta_col: str = "beta",
    se_col: str = "se",
    ...
    sharex: str = "col",
    meta: bool = True,
    combine_effects_kwargs: dict | None = None,
    ...
    for group_name in unique_groups:
        ...
        try:
            ...
            log.warning(f"  -Error in meta-analysis for group '{group_name}': {e!s}")
            continue
        ...
        if meta and HAS_STATSMODELS:
            try:
                ...
                log.warning(f"  -Could not compute heterogeneity statistics: {e!s}")
                het_stats = None
...
def plot_row(
    ...
    group_name: str,
    het_stats: tuple[float, float, float, float] | None,
    meta: bool,
    is_first_row: bool,
    fontsize: int = 12,
    font_family: str = "Arial",
...
def _simple_meta_analysis(
    ...
    # Individual studies
    for i, (b, s, name) in enumerate(zip(beta, se, row_names, strict=False)):
        results.append({
            "eff": b,
            "sd_eff": s,
            "ci_low": b - 1.96 * s,
