    ...
    if ("m" in mode) or ("r" in mode): 
        ...
        if (region is not None) and ("r" in mode):
            ...
            ax1, ax3, ax4, cbar, lead_snp_is, lead_snp_is_color = _plot_regional(
                                ...
                                pos=pos,
                                ld_link=ld_link,
                                ld_link_thr=ld_link_thr,
                                ld_link_color=ld_link_color,
                                ld_link_alpha_scale=ld_link_alpha_scale,
                                ld_link_linewidth=ld_link_linewidth,
                                ld_link_sig_level=ld_link_sig_level,
                                show_ld_score=show_ld_score,
                                sig_level=sig_level,
                                verbose=verbose,
                                log=log
                            )
