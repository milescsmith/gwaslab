...
from gwaslab.util.util_in_correct_winnerscurse import wc_correct, wc_correct_test
...
def rename_sumtats(sumstats, snplist, scaled, suffix=""):
    ...
    if scaled:
        rename_dict["MLOG10P"] = f"MLOG10P{suffix}"
    else:
        rename_dict["P"] = f"P{suffix}"
...
def rename_sumstats_full(mode, sumstats, drop, index, scaled, has_eaf, log, verbose):
    if mode=="beta" or mode=="BETA" or mode=="Beta":
        rename_dict = {
            "SNPID": "SNPID",
            ("MLOG10P" if scaled else "P"): (f"MLOG10P_{index}" if scaled else f"P_{index}"),
            "EA": f"EA_{index}",
            "NEA": f"NEA_{index}",
            "BETA": f"EFFECT_{index}",
            "SE": f"SE_{index}",
        }
    else:
        rename_dict = {
            ...
            "NEA": f"NEA_{index}",
            "OR": f"OR_{index}",
            "OR_95L": f"OR_L_{index}",
            "OR_95U": f"OR_H_{index}",
        }
    if has_eaf:
        rename_dict["EAF"] = f"EAF_{index}"
    sumstats = sumstats.rename(columns=rename_dict)

    # drop na and duplicate
    if drop==True:
        if scaled==True:
            sumstats = drop_duplicate_and_na(sumstats,  sort_by=f"MLOG10P_{index}",ascending=False, log=log , verbose=verbose)
        else:
            sumstats = drop_duplicate_and_na(sumstats,  sort_by=f"P_{index}", ascending=True, log=log , verbose=verbose)

    if scaled==True:
        sumstats.drop(f"MLOG10P_{index}",axis=1,inplace=True)
    else:
        sumstats.drop(f"P_{index}",axis=1,inplace=True)
    return sumstats
...
def check_allele_match(sig_list_merged, allele_match, label, log,verbose):
    ...
    log.write(f" -Aligned all EAs in {label[1]} with EAs in {label[0]} ...", verbose=verbose)
    if nonmatch>0:
        log.warning(f"Alleles for {nonmatch} variants do not match...")
...
def winnerscurse_correction(sig_list_merged, mode, wc_correction, sig_level, scaled1, scaled2, log, verbose):
    if mode=="beta":
        ...
        if wc_correction == "all":
            log.write(f" -Correcting BETA for winner's curse with threshold at {sig_level} for all variants...", verbose=verbose)
            ...
            log.write(f" - Correcting BETA for winner's curse with threshold at {sig_level} for significant variants...", verbose=verbose)
            sig_list_merged["EFFECT_1_RAW"] = sig_list_merged["EFFECT_1"].copy()
            sig_list_merged["EFFECT_2_aligned_RAW"] = sig_list_merged["EFFECT_2_aligned"].copy()
            log.write(f"  -Correcting BETA for {sum(match1)} variants in sumstats1...", verbose=verbose)
            ...
            log.write(f"  -Correcting BETA for {sum(match2)} variants in sumstats2...", verbose=verbose)
            ...
            log.write(f" - Correcting BETA for winner's curse with threshold at {sig_level} for significant variants in sumstats1...", verbose=verbose)
            sig_list_merged["EFFECT_1_RAW"] = sig_list_merged["EFFECT_1"].copy()
            log.write(f"  -Correcting BETA for {sum(match1)} variants in sumstats1...", verbose=verbose)
            sig_list_merged.loc[match1, "EFFECT_1"]         = sig_list_merged.loc[match1, ["EFFECT_1_RAW","SE_1"]].apply(lambda x: wc_correct_test(x[0],x[1],sig_level),axis=1)

        elif wc_correction == "sumstats2" :
            log.write(f" - Correcting BETA for winner's curse with threshold at {sig_level} for significant variants in sumstats2...", verbose=verbose)
            sig_list_merged["EFFECT_2_aligned_RAW"] = sig_list_merged["EFFECT_2_aligned"].copy()
            log.write(f"  -Correcting BETA for {sum(match2)} variants in sumstats2...", verbose=verbose)
            sig_list_merged.loc[match2, "EFFECT_2_aligned"] = sig_list_merged.loc[match2, ["EFFECT_2_aligned_RAW","SE_2"]].apply(lambda x: wc_correct_test(x[0],x[1],sig_level),axis=1)
    return sig_list_merged
...
def drop_duplicate_and_na(df,snpid="SNPID",sort_by=False,log=Log(),ascending=True,verbose=True):
    ...
    if length_before !=  length_after:
        log.write(f" -Dropped {length_before - length_after} duplicates or NAs...", verbose=verbose)
    return df
...
def configure_regression_line(is_reg,
    ...
    if is_reg is True:
        ...
        if n_valid >= 3:
            ...
            if n_valid > 0:
                log.write(f" -Too few valid pairs for regression (need ≥3, got {n_valid}). Skipping regression stats.", verbose=verbose)
        ...
        if mode=="beta" or mode=="BETA" or mode=="Beta":
            ...
            if r_se==True and n_valid >= 3:
                ...
                r_se_jackknife_string = f" ({r_se_jackknife:.2f})"
            else:
        ...
        log.write(" -H0 beta =  0",", default p = ", f"{reg[3]:.2e}", verbose=verbose)
        log.write(" -Peason correlation coefficient =  ", f"{reg[2]:.2f}", verbose=verbose)
        log.write(" -r2 =  ", f"{reg[2]**2:.2f}", verbose=verbose)
        if r_se==True and n_valid >= 3 and (mode=="beta" or mode=="BETA" or mode=="Beta"):
            log.write(f" -R se (jackknife) = {r_se_jackknife:.2e}", verbose=verbose)
        ...
        if np.isfinite(reg[0]) and reg[0] > 0:
            ...
            #add text
            try:
                p12=str(f"{p:.2e}").split("e")[0]
                pe =str(int(f"{p:.2e}".split("e")[1]))
            except:
                p12="0"
                pe="0"
            if p > 1e-300:
                p_text=r"$\mathregular{p = " + p12 + " \\times  10^{"+pe+"}}$"
            else:
                p_text="$\\mathregular{p < 1 \\times 10^{-300}}$"
            p_latex= f"{p_text}"

            if reg_text=="full":
                reg_string = "y = "+f"{reg[1]:.2f}" +" + "+ f"{reg[0]:.2f}"+" x, "+ p_latex + ", r = " +f"{reg[2]:.2f}"+r_se_jackknife_string
                ax.text(0.98,0.02,
                        reg_string,
                        va="bottom",ha="right",transform=ax.transAxes, bbox=reg_box, **font_kwargs)
            elif reg_text=="r":
                reg_string ="r = " +f"{reg[2]:.2f}"+r_se_jackknife_string
                ax.text(0.98,0.02,
                        reg_string, va="bottom",ha="right",transform=ax.transAxes, bbox=reg_box, **font_kwargs)
            elif reg_text=="r2":
                reg_string = r"$\mathregular{r^{2}} = " +f"{reg[2]**2:.2f}"
                ax.text(0.98,0.02,
                        reg_string, va="bottom",ha="right",transform=ax.transAxes, bbox=reg_box, **font_kwargs)
            ...
            if reg_text=="full":
                ax.text(0.98,0.02,
                    "y = "+f"{reg[1]:.2f}" +" - "+ f"{abs(reg[0]):.2f}"+" x, "+ p_latex + ", r = " +f"{reg[2]:.2f}"+r_se_jackknife_string,
                    va="bottom",ha="right",transform=ax.transAxes,bbox=reg_box,**font_kwargs)
            elif reg_text=="r":
                ax.text(0.98,0.02,
                        "r = " +f"{reg[2]:.2f}"+r_se_jackknife_string,
                        va="bottom",ha="right",transform=ax.transAxes, bbox=reg_box, **font_kwargs)
            elif reg_text=="r2":
                ax.text(0.98,0.02,
                        r"$\mathregular{r^{2}} = " +f"{reg[2]**2:.2f}",
                        va="bottom",ha="right",transform=ax.transAxes, bbox=reg_box, **font_kwargs)
...
def configure_legend(fig, ax, legend_mode, is_q, is_q_mc, legend_elements, legend_pos, q_level,
    ...
    if legend_mode == "full" and is_q==True :
        ...
        if is_q_mc=="fdr":
            het_label_sig = r"$\mathregular{FDR_{het} < }$" + f"{q_level}"
            het_label_sig2 = r"$\mathregular{FDR_{het} > }$" + f"{q_level}"
        elif is_q_mc=="bon":
            het_label_sig = r"$\mathregular{P_{het,bon} < }$" + f"{q_level}"
            het_label_sig2 = r"$\mathregular{P_{het,bon} > }$" + f"{q_level}"
        else:
            het_label_sig = r"$\mathregular{P_{het} < }$" + f"{q_level}"
            het_label_sig2 = r"$\mathregular{P_{het} > }$" + f"{q_level}"
    ...
    try:
        for item, label in zip(L.legend_handles, L.texts, strict=False):
...
def reorderLegend(ax=None, order=None, add=None):
    ...
    info = dict(zip(labels, handles, strict=False))
