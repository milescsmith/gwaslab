...
from gwaslab.info.g_vchange_status_polars import copy_statusp, vchange_statusp
...
def _merge_mold_with_sumstats_by_chrposp(
    ...
    if merge_by_id==False:
        ...
        log.write(f" -After merging by CHR, POS and ASET:{len(mold_sumstats)}", verbose=verbose)
    else:
        mold_sumstats = mold_sumstats.unique(subset=["SNPID","CHR","POS"])
        log.write(f" -After merging by SNPID, CHR and POS:{len(mold_sumstats)}", verbose=verbose)
    ...
    if not keep_all_variants:
        ...
        log.write(f" -Keeping all variants (including unmatched):{len(mold_sumstats)}", verbose=verbose)
