...
from gwaslab.info.g_vchange_status import ensure_status_int, status_match, vchange_status
...
def _normalize_chrom_name_vectorized(chrom_series: pd.Series, mapper: ChromosomeMapper | None = None) -> pd.Series:
    """
...
def _liftover_variant(sumstats_obj,
    ...
    if chain_path is None:
        ...
        try:
            ...
            if chain_path is None or chain_path == False:
                ...
                raise ValueError(f"No available chain file for {lifter_from_build} -> {lifter_to_build}")
            ...
            if is_builtin:
                log.log_operation(f"Using built-in chain file: {chain_path}", verbose=verbose)
            else:
                log.log_operation(f"Using chain file: {chain_path}", verbose=verbose)
        except Exception as e:
            lifter_from_build = _process_build(from_build, log=log, verbose=False)
            lifter_to_build = _process_build(to_build, log=log, verbose=False)
            raise ValueError(f"No available chain file for {lifter_from_build} -> {lifter_to_build}. Error: {e!s}")
    else:
        log.log_operation(f"Using provided chain file: {chain_path}", verbose=verbose)
    ...
    if filter_by_status and status in sumstats.columns:
        ...
        log.log_operation(f"Converting variants with status code xxx0xxx: {to_lift_count:,}", verbose=verbose)
    ...
    if to_build is None:
        ...
        elif "hg38" in chain_name.lower() or "grch38" in chain_name.lower():
            to_build = "38"
