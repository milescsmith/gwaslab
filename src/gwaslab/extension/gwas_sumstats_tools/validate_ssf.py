...
def validate_ssf_file(
    ...
    try:
        ...
        if verbose:
            log.write("Validating minimum row count...", verbose=verbose)
...
def _detect_effect_field(header: list[str]) -> str | None:
...
def _detect_p_value_field(header: list[str]) -> str | None:
