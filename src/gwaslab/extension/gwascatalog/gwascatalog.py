...
def _v2_cache_path(cache_dir: str, efo: str, sig_level: float, show_child_traits: bool) -> str:
    ...
    suffix = hashlib.md5(f"{sig_level}_{show_child_traits}".encode()).hexdigest()[:8]
    name = f"GWASCatalog_v2_{efo}_associationsByTraitSummary_text_{datestring}_{suffix}.json"
...
class GWASCatalogClient:
    ...
    def __init__(self, timeout: int = 30, log: Log | None = None, verbose: bool = True,
    ...
    def _get_all_pages(
        ...
        while current_page < total_pages:
            ...
            if data and "_embedded" in data:
                ...
                elif (current_page + 1) % 20 == 0 or (current_page + 1) == total_pages:
    ...
    def get_studies(
        ...
        gxe: bool | None = None,
        show_child_traits: bool | None = None,
        page: int | None = None,
        size: int | None = None,
        get_all: bool = False
    ...
    def get_associations(
        ...
        show_child_traits: bool | None = None,
        extended_geneset: bool | None = None,
        page: int | None = None,
        size: int | None = None,
        get_all: bool = False
    ) -> Union[pd.DataFrame, dict[str, Any], list[dict[str, Any]]]:
    ...
    def get_all_variants_for_trait(self, trait_name: str) -> set:
        ...
        if isinstance(associations, pd.DataFrame):
            ...
            for association in associations:
                if association.get("snp_effect_allele"):
    ...
    def get_all_genes_for_trait(self, trait_name: str) -> set:
        ...
        if isinstance(associations, pd.DataFrame):
            ...
            for association in associations:
                if association.get("mapped_genes"):
    ...
    def get_known_variants_for_trait(
        ...
        """
        self.log.write(f" -Querying GWAS Catalog API v2 for trait: {efo}...", verbose=verbose)
            ...
            if os.path.isfile(p):
                try:
                    ...
                    if isinstance(df, pd.DataFrame):
                        self.log.write(f" -Loaded {len(df)} variants from cache: {p}", verbose=verbose)
                        return df
                except Exception:
                    pass
        ...
        try:
            ...
            if (not isinstance(associations_df, pd.DataFrame) or len(associations_df) == 0) and efo.startswith("MONDO_"):
                ...
                try:
                    ...
                    if known_Sumstats and hasattr(known_Sumstats, "data") and len(known_Sumstats.data) > 0:
                        self.log.write(f" -Retrieved {len(known_Sumstats.data)} variants using old API v1", verbose=verbose)
                        return known_Sumstats.data.copy()
                except Exception as e2:
                    self.log.write(f" -Fallback to old API also failed: {e2!s}", verbose=verbose)

            if not isinstance(associations_df, pd.DataFrame) or len(associations_df) == 0:
                self.log.write(f" -No associations found for trait: {efo}", verbose=verbose)
                if use_cache:
                    try:
            ...
            self.log.write(f" -Retrieved {len(associations_df)} associations from API", verbose=verbose)

            # Debug: show available columns
            if verbose:
                self.log.write(f" -Available columns: {list(associations_df.columns)[:10]}", verbose=verbose)
            ...
            if pval_col:
                ...
                self.log.write(f" -Filtered to {len(associations_df)} associations with p < {sig_level} (from {before_filter})", verbose=verbose)
            else:
            ...
            for idx, row in associations_df.iterrows():
                try:
                    ...
                    self.log.write(f" -Warning: Error processing association row {idx}: {e!s}", verbose=verbose)
                    continue

            if skipped_no_rsid > 0:
                self.log.write(f" -Skipped {skipped_no_rsid} associations without valid rsID", verbose=verbose)
            if skipped_no_location > 0:
                self.log.write(f" -Skipped {skipped_no_location} associations without valid CHR/POS", verbose=verbose)
            ...
            self.log.write(f" -Successfully extracted {len(result_df)} variants with CHR/POS", verbose=verbose)
            if use_cache:
                try:
                    os.makedirs(cache_dir, exist_ok=True)
            ...
            self.log.write(f" -Error retrieving data from GWAS Catalog API v2: {e!s}. Falling back to old API...", verbose=verbose)
            # Fallback to old API
            try:
                from gwaslab.util.util_ex_gwascatalog import gwascatalog_trait
                known_Sumstats = gwascatalog_trait(efo, source="NCBI", sig_level=sig_level,
