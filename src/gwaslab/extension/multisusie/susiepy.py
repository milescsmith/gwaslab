...
def multisusie(
    ...
    assert np.all([X.shape[0] == Y.shape[0] for X,Y in zip(X_list, Y_list, strict=False)])
    ...
    Y_list = [Y-mean_Y for Y,mean_Y in zip(Y_list, mean_y_list, strict=False)]
    ...
    if low_memory_mode:
        ...
        X_std_list = [(X-cm)/csd for X,cm in zip(X_list, cm_arr, strict=False)]
...
def compute_lbf(V, Y_list, X_std_list, X_l2_arr,
    ...
    if np.all(np.isclose(V, 0)):
        ...
        YT_invD_Z = np.array([Y.dot(X)/sigma2 for Y,X,sigma2 in zip(Y_list, X_std_list, residual_variance, strict=False)], dtype=float_type)
...
def single_effect_regression(Y_list, X_std_list, V, X_l2_arr, w_pop,
    ...
    loglik = lbf_model + np.sum([np.sum(stats.norm(0, np.sqrt(sigma2)).logpdf(Y)) for sigma2,Y in zip(residual_variance, Y_list, strict=False)])
