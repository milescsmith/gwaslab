...
def _read_ref_ld(args, log):
    ...
    log.log(
        f"  -Read reference panel LD Scores for {len(ref_ld)} SNPs.")
    return ref_ld
...
def _read_w_ld(args, log):
    ...
    log.log(
        f"  -Read regression weight LD Scores for {len(w_ld)} SNPs.")
    return w_ld
...
def _print_cov(ldscore_reg, ofh, log):
    ...
    log.log(
        f"  -Printing covariance matrix of the estimates to {ofh}.")
...
def cell_type_specific(sumstats, args, log):
    ...
    log.log(f"  -Removed {n_snp-np.sum(ii)} SNPs with chi^2 > {chisq_max} ({np.sum(ii)} SNPs remain)")
...
def estimate_rg(sumstats, other_sumstats, args, log):
    ...
    for i, p2 in enumerate(other_sumstats):
        log.log(
            f"  -Computing rg for phenotype {i + 2}/{len(rg_paths)}")
        try:
...
def _print_gencor(args, log, rghat, ref_ld_cnames, i, rg_paths, print_hsq1):
    ...
    log.log(
        l(log_prefix_short+f"Heritability of phenotype {i + 2}/{len(rg_paths)}"))
...
def _check_arg_len(x, n):
    ...
    if len(x) != n:
        raise ValueError(
            f"{m} must have the same number of arguments as --rg/--h2.")
