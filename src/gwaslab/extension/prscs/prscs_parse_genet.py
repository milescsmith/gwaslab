...
def parse_sumstats(ref_dict, vld_dict, sst_file, n_subj, log):
    ...
    vld_snp = set(zip(vld_snp_list, vld_a1_list, vld_a2_list, strict=False))

    ref_snp = set(zip(ref_snp_list, ref_a1_list, ref_a2_list, strict=False)) | \
              set(zip(ref_snp_list, ref_a2_list, ref_a1_list, strict=False)) | \
              set(zip(ref_snp_list, [mapping[aa] for aa in ref_a1_list], [mapping[aa] for aa in ref_a2_list], strict=False)) | \
              set(zip(ref_snp_list, [mapping[aa] for aa in ref_a2_list], [mapping[aa] for aa in ref_a1_list], strict=False))

    sst_snp = set(zip(sst_dict_temp["SNP"], sst_dict_temp["A1"], sst_dict_temp["A2"], strict=False)) | \
              set(zip(sst_dict_temp["SNP"], sst_dict_temp["A2"], sst_dict_temp["A1"], strict=False)) | \
              set(zip(sst_dict_temp["SNP"], [mapping[aa] for aa in sst_dict_temp["A1"]], [mapping[aa] for aa in sst_dict_temp["A2"]], strict=False)) | \
              set(zip(sst_dict_temp["SNP"], [mapping[aa] for aa in sst_dict_temp["A2"]], [mapping[aa] for aa in sst_dict_temp["A1"]], strict=False))
