...
from gwaslab.qc.qc_check_datatype import check_dataframe_memory_usage, check_datatype
...
def _read_pipcs(data_or_dataframe,
    ...
    log.write(f" -File:{output_prefix}",verbose=verbose)
    ...
    if "@" in output_prefix:
        ...
        for index,pipcs_path in enumerate(pipcs_path_list):
            log.write(f" -Loading {pipcs_loci_list[index]}:" + pipcs_path)
        ...
        pipcs = pd.read_csv(f"{output_prefix}",**readcsv_kwargs)
