...
def _match_sumstats_with_ref_bim(sumstats: pd.DataFrame,
    ...
    log.write(f" -Detected sumstats chromosome format: {sumstats_format}",verbose=verbose)
    ...
    if n_invalid_bim > 0:
        log.write(f" -Warning: {n_invalid_bim} BIM variants have unconvertible chromosome notation.",verbose=verbose)
    ...
    sumstats_chr_pos = set(zip(sumstats["CHR"], sumstats["POS"], strict=False))

    # Create a boolean mask for filtering - more efficient than apply
    ref_bim_filtered["_chr_pos_tuple"] = list(zip(ref_bim_filtered["_CHR_bim_sumstats"], ref_bim_filtered["POS_bim"], strict=False))
    ...
    log.write(f" -Filtered reference BIM from {len(ref_bim_all)} to {len(ref_bim_matched)} variants matching sumstats CHR/POS...",verbose=verbose)
    ...
    if has_allele_info:
        ...
        log.write(f" -Matched {n_matched} variants using CHR, POS, EA, NEA...",verbose=verbose)
    else:
        ...
        log.write(f" -Matched {n_matched} variants using CHR, POS only...",verbose=verbose)
