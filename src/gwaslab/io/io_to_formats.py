...
from gwaslab.bd.bd_common_data import get_format_inverse_for_export, get_formats_list, get_number_to_chr
...
from gwaslab.util.util_in_filter_value import _exclude, _exclude_hla, _extract
...
def _to_format(sumstats_or_dataframe,
    ...
    if hapmap3==True:
        ...
        after = len(output)
        onetime_log.write(f" -Extract {after} variants in Hapmap3 datasets for build {build}.",verbose=verbose)
    ...
    if output_log is True:
        ...
        onetime_log.write(f" -Saving log file to: {log_path}",verbose=verbose)
...
def tofmt(sumstats,
    ...
    ####################################################################################################################
        ...
        log.write(f" -Writing sumstats to: {path}...",verbose=verbose)
        try:
            fast_to_vcf(sumstats, path, vcf_header, output_format, meta_data, meta)
        except:
            log.write("Error in using fast_to_vcf. Falling back to original implementation.",verbose=verbose)
            # output header
            ...
            with open(path,"a") as file:
                ...
                for index,row in sumstats.iterrows():
                    ...
                    file.write(f"{CHROM}\t{POS}\t{ID}\t{REF}\t{ALT}\t{QUAL}\t{FILTER}\t{INFO}\t{FORMAT}\t{DATA}\n")
    ...
    ####################################################################################################################
        ...
        if gzip ==True and tab_fmt not in non_gzip_tab_fmt:
            path = path + "."+suffix+f".{tab_fmt}.gz"
        else:
            path = path + "."+suffix+f".{tab_fmt}"

        yaml_path = path + "."+suffix+f".{tab_fmt}-meta.yaml"
...
def _write_tabular(sumstats,rename_dictionary, path, tab_fmt, to_csvargs, to_tabular_kwargs, log, verbose, gzip):
    if tab_fmt=="tsv" or tab_fmt=="csv":
        ...
        if "@" in path:
            ...
            log.write("  -@ detected: writing each chromosome to a single file...",verbose=verbose)
            log.write("  -Chromosomes:{}...".format(list(sumstats["CHR"].unique())),verbose=verbose)
            for single_chr in list(sumstats["CHR"].unique()):
                single_path = path.replace("@",f"{single_chr}")
        ...
        if to_tabular_kwargs and "partition_cols" in to_tabular_kwargs and to_tabular_kwargs["partition_cols"]:
            ...
            log.write(f"  -Partitioned parquet will be written to directory: {path}", verbose=verbose)
...
def _configure_ssf_meta(sumstats, fmt, ssfmeta, meta, meta_data, path, md5_value, yaml_path, log, verbose):
    ...
    if ssfmeta==True:
        ...
        log.write(f" -Exporting SSF-style meta data to {yaml_path}",verbose=verbose)
...
def md5sum_file(filename,log,verbose):
    ...
    with open(filename+".md5sum","w") as f:
        ...
        log.write(f" -md5sum: {out}",verbose=verbose)
        return out
...
def _process_vcf_header(sumstats, meta, meta_data, build, log, verbose):
    ...
    log.write(f"  -TotalVariants:{len(sumstats)}",verbose=verbose)
    log.write(f"  -HarmonisedVariants:{harmonised}",verbose=verbose)
    log.write(f"  -VariantsNotHarmonised:{len(sumstats)-harmonised}",verbose=verbose)
    log.write(f"  -SwitchedAlleles:{switchedalleles}",verbose=verbose)
