...
from gwaslab.hm.hm_casting_polars import (
    _align_with_moldp,
    _fill_missing_columnsp,
    _merge_mold_with_sumstats_by_chrposp,
    _renaming_colsp,
    _sort_pair_colsp,
)
from gwaslab.info.g_Log import Log
from gwaslab.info.g_meta import _init_meta, _update_meta
from gwaslab.io.io_input_type import _get_id_column
from gwaslab.io.io_to_pickle import _offload, _reload, dump_pickle_multi
from gwaslab.qc.qc_check_datatype import check_dataframe_shape, check_datatype
from gwaslab.qc.qc_check_datatype_polars import check_dataframe_shape_polars, check_datatype_polars
from gwaslab.qc.qc_fix_sumstats import _flip_allele_stats, _process_build
...
class SumstatsMulti:
    def __init__(
        ...
        for i,sumstatsObject in enumerate(sumstatsObjects):
            if not isinstance(sumstatsObject, Sumstats):
                if not isinstance(sumstatsObject, Sumstatsp):
                    raise ValueError(f"Please provide GWASLab Sumstats Object #{i+1}.")
        ...
        for i,sumstatsObject in enumerate(sumstatsObjects):
            self.log.write( f" -Checking sumstats Object #{i+1}...", verbose=verbose)
            ...
            if sumstatsObject.meta["gwaslab"]["study_name"] in self.names:
                ...
                self.log.write( f"  -Sumstats Object #{i+1} name: {new_study_name}", verbose=verbose)
        ...
        self.log.write( f" -Variant Info columns: {self.snp_info_cols[0]}" , verbose=verbose)
        for i in range(len(sumstatsObjects)):
            self.log.write( f" -Sumstats #{i+1} variant statistics columns: {self.stats_cols[i]}" , verbose=verbose)
            self.log.write( f" -Sumstats #{i+1} other columns: {self.other_cols[i]}" , verbose=verbose)
        ...
        if engine=="polars":
            ...
            for i, sumstatsObject in enumerate(sumstatsObjects):
                if i >0:
                    self.log.write(f"Merging Sumstats #{i+1} to main DataFrame...")
                    ...
                    self.log.write(f"Finished merging Sumstats #{i+1} to main DataFrame.")
        else:
            for i, sumstatsObject in enumerate(sumstatsObjects):
                if i >0:
                    self.log.write(f"Merging Sumstats #{i+1} to main DataFrame...")
    ...
    def _merge_two_sumstats(
        ...
        molded_sumstats = _sort_pair_cols(molded_sumstats, verbose=verbose, log=self.log, suffixes=[f"_{j}" for j in range(1,i+2)])
    ...
    def get_lead(self, build: str | None = None, gls: bool = False, **kwargs: Any) -> Union[pd.DataFrame, "SumstatsMulti"]:
    ...
    def reload(self, delete_files: list[str] | None = None) -> None:
        """
