...
class RScriptRunner:
    ...
    def __init__(
        ...
        r: str = "Rscript",
        log: Log | None = None,
        timeout: float | None = 86400,  # 24 hours in seconds
        temp_dir: str | None = None,
        cleanup: bool = True
    ):
        """
    ...
    def execute(
        ...
        try:
            ...
            result.success = False
            result.execution_time = time.time() - start_time
            result.errors.append(f"Unexpected error during R script execution: {e!s}")
            result.output = str(e)
            self.log.warning(f"Unexpected error during R script execution: {e!s}", verbose=verbose)
    ...
    def _collect_output_files(
        ...
        for expected_file in expected_outputs:
            ...
            if not os.path.exists(actual_path):
                ...
                elif os.path.exists(expected_file_normalized):
