...
from gwaslab.extension.ldsc.ldsc_sumstats import cell_type_specific, estimate_h2, estimate_rg
from gwaslab.info.g_Log import Log
from gwaslab.io.io_read_ldsc import parse_ldsc_summary, parse_partitioned_ldsc_summary
from gwaslab.util.util_in_filter_value import _exclude_hla, _exclude_sexchr, _filter_palindromic, _filter_values
...
def _estimate_h2_by_ldsc(
    ...
    verbose: bool = True,
    munge: bool = False,
    munge_kwargs: dict[str, Any] | None = None,
    **raw_kwargs: Any
    ...
    **raw_kwargs
    ...
    for key, value in kwargs.items():
        log.write(f"  -{key}:{value}", verbose=verbose)
...
def _munge_sumstats(
    ...
    **kwargs
    ...
    if rename_dict:
        sumstats = sumstats.rename(columns=rename_dict)
        log.write(f" -Renamed columns: {rename_dict}", verbose=verbose)
    ...
    if "P" in sumstats.columns:
        ...
        bad_p = (~valid_p).sum()
        if bad_p > 0:
            log.write(f" -WARNING: {bad_p} SNPs had P outside of (0,1]. The P column may be mislabeled.", verbose=verbose)
        sumstats = sumstats.loc[valid_p].copy()
        log.write(f" -After P-value filtering: {len(sumstats)} SNPs remain", verbose=verbose)
    ...
    if "INFO" in sumstats.columns:
        ...
        if bad_info_count > 0:
            log.write(f" -WARNING: {bad_info_count} SNPs had INFO outside of [0,1.5]. The INFO column may be mislabeled.", verbose=verbose)

        sumstats = sumstats.loc[valid_info].copy()
        log.write(f" -After INFO filtering (>= {info}): {len(sumstats)} SNPs remain", verbose=verbose)
    ...
    if "FRQ" in sumstats.columns:
        ...
        if bad_frq_count > 0:
            log.write(f" -WARNING: {bad_frq_count} SNPs had FRQ outside of [0,1]. The FRQ column may be mislabeled.", verbose=verbose)
        ...
        log.write(f" -After MAF filtering (>= {maf}): {len(sumstats)} SNPs remain", verbose=verbose)
    ...
    if allele1_col and allele2_col:
        ...
        log.write(f" -After allele filtering (strand-unambiguous): {len(sumstats)} SNPs remain", verbose=verbose)
    ...
    if nopalindromic and allele1_col and allele2_col:
        ...
        log.write(f" -After removing palindromic SNPs: {len(sumstats)} SNPs remain", verbose=verbose)
    ...
    if "N" in sumstats.columns:
        ...
        log.write(f" -After N filtering (>= {min_n}): {len(sumstats)} SNPs remain", verbose=verbose)
    ...
    if exclude_hla and "CHR" in sumstats.columns and "POS" in sumstats.columns:
        ...
        log.write(f" -After excluding HLA: {len(sumstats)} SNPs remain", verbose=verbose)
    ...
    if exclude_sexchr and "CHR" in sumstats.columns:
        ...
        log.write(f" -After excluding sex chromosomes: {len(sumstats)} SNPs remain", verbose=verbose)
    ...
    if "SNP" in sumstats.columns:
        ...
        if old_len != new_len:
            log.write(f" -Removed {old_len - new_len} SNPs with duplicated SNP IDs ({new_len} SNPs remain)", verbose=verbose)

    # Final summary
    log.write(f" -Munging complete: {original_len - len(sumstats)} SNPs removed, {len(sumstats)} SNPs remain (from original {original_len})", verbose=verbose)
