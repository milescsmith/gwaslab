...
class PythonScriptRunner:
    ...
    def execute(
        ...
        try:
            ...
            result.success = False
            result.execution_time = time.time() - start_time
            result.errors.append(f"Unexpected error during Python script execution: {e!s}")
            result.output = str(e)
            self.log.warning(f"Unexpected error during Python script execution: {e!s}", verbose=verbose)
