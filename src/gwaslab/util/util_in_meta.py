...
def meta_analyze_multi(
    ...
    if n_before > n_after:
        log.write(f" -Removed {n_before - n_after} duplicate variants before meta-analysis", verbose=True)
    ...
    log.write(f" -Iterating through {nstudy} datasets to compute statistics for fixed-effect model...")
    for i in range(nstudy):
        ...
        valid_mask = (
            ~sumstats_multi[f"BETA_{i+1}"].isna() &
    ...
    if random_effects==True:
        log.write(f" -Iterating through {nstudy} datasets to compute statistics for random-effects model...")
