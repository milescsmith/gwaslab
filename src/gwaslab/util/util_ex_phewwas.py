...
def _extract_scalar_value(value: Any) -> Any | None:
    """
...
def _extract_risk_allele(snp_effect_allele: Any) -> str | None:
    """
...
def _extract_genes(mapped_genes: Any) -> str | None:
    """
...
def _extract_chr_pos_from_locations(locations: Any) -> tuple[str | None, int | None]:
    """
...
def _extract_associations(
    ...
    rsid: str = "rsID",
                         ...
                         fetch_traits: bool | None = None,
                         fetch_studies: bool | None = None,
                         fetch_variants: bool | None = None,
                         gcv2_columns: list | None = None,
                         use_gcv2_format: bool = True) -> tuple[pd.DataFrame | None, pd.DataFrame | None]:
    """
...
def _fetch_associations(
    ...
    for index, row in unique_sumstats.iterrows():
        ...
        try:
            ...
            log.warning(f"Error fetching associations for {rs_id}: {e!s}", verbose=verbose)
            empty_variants.append(rs_id)
...
def _extract_study(client, row: pd.Series, association_id: str,
                   log: Log, verbose: bool) -> dict | None:
    """
    ...
    try:
        ...
        log.warning(f"Error fetching study for {study_id}: {e!s}", verbose=verbose)
        return None
...
def _extract_variant(client, row: pd.Series, association_id: str, rsid: str,
    ...
    try:
        ...
        log.warning(f"Error fetching variant for {rs_id_for_variant}: {e!s}", verbose=verbose)
        return None
...
def _extract_numeric_from_string(value: Any) -> float | None:
    """
