...
def process_vcf_to_hfd5(vcf: str,
                                 ...
                                 chr_dict: dict[str, str] | None = None,
                                 complevel: int = 3,
                                 threads: int = 1,
                                 chr_list: list[Union[int, str]] | None = None,
    ...
    log.write(f" -Reference VCF path: {vcf}", verbose=verbose)
    ...
    log.write(f" -Compression level: {complevel}", verbose=verbose)
    log.write(f" -Number of threads: {threads}", verbose=verbose)
    log.write(f" -Overwrite existing files: {overwrite}", verbose=verbose)
    ...
    h5_dir = directory
    log_path = f"{directory}/{vcf_file_name_base}.rsID_CHR_POS_mod10.log"
    log.write(f" -HDF5 Output directory: {h5_dir}", verbose=verbose)
    log.write(f" -HDF5 File pattern: {vcf_file_name_base}.chr{{chr_num}}.rsID_CHR_POS_mod10.h5", verbose=verbose)
    log.write(f" -Log output path: {log_path}", verbose=verbose)
    ...
    if threads > 1:
        ...
        log.write(f" -Processing {len(chr_list_vcf)} chromosomes in parallel with {threads} threads...", verbose=verbose)
        ...
        with Pool(threads) as pool:
            ...
            log.write(f" -Total rows processed: {total_rows}", verbose=verbose)
    else:
