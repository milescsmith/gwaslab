...
def get_power(
              mode: str = "b",
              genotype_rr: float | None = None,
              genotype_or: float | None = None,
              beta: float = 0.3,
              eaf: float = 0.1,
              n: int = 10000,
              ncase: int = 2000,
    ...
    if mode=="b":
        ...
        log.write(f"  -Number of cases:{ncase}", verbose=verbose)
        log.write(f"  -Number of controls:{ncontrol}", verbose=verbose)
        if genotype_rr is not None:
            log.write(f"  -Risk allele RR:{genotype_rr}", verbose=verbose)
        elif genotype_or is not None:
            log.write(f"  -Risk allele OR:{genotype_or}", verbose=verbose)
        elif beta is not None:
            log.write(f"  -Risk allele beta:{beta}", verbose=verbose)
        else:
            genotype_rr = 0.1
            log.write(f"  -Risk allele RR:{genotype_rr}", verbose=verbose)

        log.write(f"  -Disease prevalence:{prevalence}", verbose=verbose)
        log.write(f"  -Risk allele frequency: {daf}", verbose=verbose)
        ...
        if genotype_rr is None:
            ...
            if or_to_rr ==False:
                ...
                log.write(f" -OR is converted to GRR using base prevalence: {prevalence}", verbose=verbose)
        ...
        log.write(f" - Individuals with AA genotype: {aap}", verbose=verbose)
        log.write(f" - Individuals with AB genotype: {abp}", verbose=verbose)
        log.write(f" - Individuals with BB genotype: {bbp}", verbose=verbose)
        ...
        log.write(f" - In cases: {pcase}", verbose=verbose)
        log.write(f" - In controls: {pcontrol}", verbose=verbose)
        ...
        log.write(f"Expected power: {power}", verbose=verbose)
        ...
        log.write(f"  -Significance level: {sig_level}", verbose=verbose)
        log.write(f"  -EAF: {eaf}", verbose=verbose)
        log.write(f"  -BETA: {beta}", verbose=verbose)
        log.write(f"  -N: {n}", verbose=verbose)
        log.write(f"  -SNPR2: {2*eaf*(1-eaf)*(beta**2)}", verbose=verbose)
...
def get_beta(
    ...
    if mode=="q":
        if t >0:
            ...
            log.write(f" -Extracting eaf-beta combinations with power = {t}...", verbose=verbose)
            i,j=1,1
            eaf_beta = []
