...
def _run_prscs(
         ...
         sst_file: pd.DataFrame | None = None,
            a: float = 1,
            b: float = 0.5,
            phi: float | None = None,
    ...
    if max_threads > 1:
        log.write(f"Running PRS-CS in parallel mode with {max_threads} threads for {n_chrom} chromosomes")
        ...
        log.write(f"Completed processing {len(results)} chromosomes in parallel")
    else:
