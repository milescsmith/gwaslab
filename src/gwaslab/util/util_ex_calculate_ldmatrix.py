    ...
    **kwargs: Any
    ...
    if loci_chrpos is None:
        ...
        for chrpos in loci_chrpos:
            chrpos_row_dict={}
            chrpos_row_dict["SNPID"]=f"{chrpos[0]}:{chrpos[1]}"
    ...
    log.write(f" -plink1.9 path: {plink}",verbose=verbose)
    log.write(f" -plink2 path: {plink2}",verbose=verbose)
...
def _export_snplist_and_locus_sumstats(
    ...
    suffixes: list[str] | None = None
        ...
        log.write(f" -Exporting SNP list of {len(matched_sumstats)}  to: {matched_snp_list_path}...")
        ...
        log.write(f" -Exporting locus sumstats to: {matched_sumstats_path}...")
