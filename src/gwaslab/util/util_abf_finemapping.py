...
from gwaslab.util.util_in_filter_value import _get_flanking_by_chrpos, _get_flanking_by_id
...
def calc_abf(
    ...
    **kwargs: Any
    ...
    log.write(f"Start to calculate approximate Bayesian factor for {len(insumstats)} variants",verbose=verbose)
    ...
    log.write(f" - Priors for the standard deviation W of the effect size parameter β : {w} ",verbose=verbose)
    # binary -> w=0.2
    # quant  -> w=0.15
    omega = w**2
    se = insumstats["SE"]
...
def calc_PIP(
    ...
    **kwargs: Any
    ...
    insumstats = insumstats.copy()
    log.write(f"Start to calculate PIP for {len(insumstats)} variants",verbose=verbose)
...
def _abf_finemapping(
    ...
    chrpos: tuple[int, int] | None = None,
    snpid: str | None = None,
    log: Log = Log(),
    **kwargs: Any
) -> pd.DataFrame:
...
def _make_cs(
    ...
    for index, row in insumstats.iterrows():
        ...
        if pip_sum > threshold:
            break
    log.write(f"Finished constructing a {threshold * 100!s}% credible set with {len(cs)!s} variant(s)",verbose=verbose)
    return cs
