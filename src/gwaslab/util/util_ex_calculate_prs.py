...
from gwaslab.io.io_plink import _load_bim_single, _load_pvar_single, _process_plink_input_files
...
def _calculate_prs(sumstats_or_dataframe: Union["Sumstats", pd.DataFrame],
    ...
    #matching_alleles
        ...
        for index, chrom in enumerate(chrlist):
            ...
            log.write(f"  -Model file is saved to : {model_path} .")
...
def _run_calculate_prs(
    ...
    log.write(f" -Start to calculate PRS for Chr {chrom}...")
    ...
    if filetype=="bfile":
        file_flag = f"--bfile {bpfile_to_use}"
    else:
        file_flag = f"--pfile {bpfile_to_use}"
...
def _match_snpid_with_bim(
    ...
    #load bim
    log.write(f"   -#variants in model for Chr {chrom} : {len(chr_sumstats)}")
    if filetype == "bfile":
    ...
    log.write(f"   -#variants matched for Chr {chrom} : {len(matched_sumstats)}")
