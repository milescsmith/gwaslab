...
def _run_multisusie_rss_from_file(
    ...
    **kwargs : Any
    ...
    log.write(f" -Loading filelist from: {filepath}", verbose=kwargs.get("verbose", True))
    ...
    log.write(f" -Found {len(unique_loci)} unique locus/loci", verbose=kwargs.get("verbose", True))
    ...
    log.write(f" -Number of studies: {nstudy}", verbose=kwargs.get("verbose", True))
    ...
    for locus_idx, locus in enumerate(unique_loci):
        ...
        if len(locus_rows) != nstudy:
            log.warning(f"  -Locus {locus} has {len(locus_rows)} rows, expected {nstudy}. Skipping...", verbose=kwargs.get("verbose", True))
            continue
        ...
        for idx, row in locus_rows.iterrows():
            ...
            if not os.path.exists(ld_path):
                log.warning(f"  -LD matrix file not found: {ld_path}. Skipping locus...",
                           verbose=kwargs.get("verbose", True))
                break

            log.write(f"  -Loading LD matrix from: {ld_path}", verbose=kwargs.get("verbose", True))
            ...
            if not os.path.exists(sumstats_path):
                log.warning(f"  -Sumstats file not found: {sumstats_path}. Skipping locus...",
                           verbose=kwargs.get("verbose", True))
                break

            log.write(f"  -Loading sumstats from: {sumstats_path}", verbose=kwargs.get("verbose", True))
            ...
            if "SNP" in sumstats.columns:
                ...
                log.warning(f"  -Available columns in sumstats: {list(sumstats.columns)}",
                           verbose=kwargs.get("verbose", True))
                raise ValueError(f"SNP or SNPID column not found in sumstats file: {sumstats_path}. Available columns: {list(sumstats.columns)}")
            ...
            if "Beta" in sumstats.columns:
                ...
                raise ValueError(f"BETA column not found in sumstats file: {sumstats_path}")
            ...
            if "Se" in sumstats.columns:
                ...
                raise ValueError(f"SE column not found in sumstats file: {sumstats_path}")
        ...
        if len(R_list) != nstudy or len(b_list) != nstudy:
            log.warning(f"  -Failed to load all data for locus {locus}. Skipping...",
                       verbose=kwargs.get("verbose", True))
            continue
        ...
        if len(set(len(vids) for vids in variant_ids_list)) > 1:
            log.warning(f"  -Variant lists have different lengths for locus {locus}. Using first study's variants...",
                       verbose=kwargs.get("verbose", True))
        ...
        for i, R in enumerate(R_list):
            if R.shape[0] != n_variants or R.shape[1] != n_variants:
                log.warning(f"  -LD matrix {i+1} has shape {R.shape} but expected ({n_variants}, {n_variants}) for locus {locus}. Skipping...", verbose=kwargs.get("verbose", True))
                break
        else:
            # All checks passed, run MultiSuSiE
            log.write(f"  -Running MultiSuSiE for locus {locus} ({n_variants} variants)...",
                     verbose=kwargs.get("verbose", True))
            ...
            if varY_list is None:
                ...
                if len(varY_list) != nstudy:
                    log.warning(f"  -varY_list length ({len(varY_list)}) does not match number of studies ({nstudy}). Using first {nstudy} values...", verbose=kwargs.get("verbose", True))
    ...
    log.write(f" -Combined results from {len(all_results)} loci: {len(combined_results)} total variants", verbose=kwargs.get("verbose", True))
    ...
    **kwargs : Any
    ...
    if b_list is None or s_list is None:
        ...
        for i in range(nstudy):
            ...
            if beta_col not in data.columns or se_col not in data.columns:
                raise ValueError(
                    f"BETA_{i+1} or SE_{i+1} columns not found in data. "
                    "Please ensure summary statistics are properly formatted."
                )
            ...
            log.write(f"  -Study {i+1}: Extracted {len(beta_raw)} variants, {np.isnan(beta_raw).sum()} NaN in BETA, {np.isnan(se_raw).sum()} NaN in SE", verbose=kwargs.get("verbose", True))

            # Log causal variants before conversion
            if variant_ids is not None and len(variant_ids) > 38:
                causal_indices = [3, 10, 38]
                log.write(f"  -Study {i+1}: Causal variants (indices 3, 10, 38) BEFORE NaN conversion:",
                         verbose=kwargs.get("verbose", True))
                for idx in causal_indices:
                    ...
                    log.write(f"    Index {idx}: BETA={beta_val}, SE={se_val}",
                             verbose=kwargs.get("verbose", True))
            ...
            if variant_ids is not None and len(variant_ids) > 38:
                causal_indices = [3, 10, 38]
                log.write(f"  -Study {i+1}: Causal variants (indices 3, 10, 38) AFTER NaN conversion:",
                         verbose=kwargs.get("verbose", True))
                for idx in causal_indices:
                    log.write(f"    Index {idx}: BETA={beta[idx]:.6f}, SE={se[idx]:.6f}",
                             verbose=kwargs.get("verbose", True))
    ...
    if population_sizes is None:
        ...
        for i in range(nstudy):
            ...
            if n_col in data.columns:
                ...
                if pd.isna(n_val):
                    raise ValueError(f"N_{i+1} column contains only NaN values")
                ...
                if "n" in study_meta.get("gwaslab", {}):
                    ...
                    raise ValueError(
                        f"N_{i+1} column not found and sample size not available in meta"
                    )

    log.write(f" -Population sizes: {population_sizes}", verbose=kwargs.get("verbose", True))
    ...
    if varY_list is None:
        ...
        for i in range(nstudy):
            ...
            # varY ≈ (BETA^2 + SE^2) for standardized traits
    ...
    if len(R_list) != nstudy:
        raise ValueError(
            f"Number of LD matrices ({len(R_list)}) does not match number of studies ({nstudy})"
        )
    ...
    for i, R in enumerate(R_list):
        if R.shape[0] != n_variants or R.shape[1] != n_variants:
            raise ValueError(
                f"LD matrix {i+1} has shape {R.shape} but expected ({n_variants}, {n_variants})"
            )
    ...
    if np.isscalar(rho):
        ...
        np.fill_diagonal(rho_matrix, 1.0)
        rho = rho_matrix
        log.write(f"  -Converted scalar rho={rho_original} to {K}x{K} correlation matrix", verbose=kwargs.get("verbose", True))
        log.write(f"  -Rho matrix:\n{rho}", verbose=kwargs.get("verbose", True))
    elif isinstance(rho, (list, np.ndarray)):
        # Ensure it's a numpy array
        rho = np.asarray(rho, dtype=float_type)
        log.write(f"  -Rho is already a matrix with shape {rho.shape}",
                 verbose=kwargs.get("verbose", True))
    ...
    log.write(f"    -Number of variants: {len(variant_ids)}", verbose=kwargs.get("verbose", True))
    log.write(f"    -Population sizes: {population_sizes}", verbose=kwargs.get("verbose", True))
    log.write("    -varY_list: {}".format([f"{v:.6f}" for v in varY_list]), verbose=kwargs.get("verbose", True))
    log.write(f"    -R_list shapes: {[R.shape for R in R_list]}", verbose=kwargs.get("verbose", True))
    log.write(f"    -b_list shapes: {[b.shape for b in b_list]}", verbose=kwargs.get("verbose", True))
    log.write(f"    -s_list shapes: {[s.shape for s in s_list]}", verbose=kwargs.get("verbose", True))
    ...
    if len(variant_ids) > 38:
        ...
        for idx in causal_indices:
            ...
            for pop_idx in range(nstudy):
                log.write(f"      Pop {pop_idx+1}: BETA={b_list[pop_idx][idx]:.6f}, SE={s_list[pop_idx][idx]:.6f}", verbose=kwargs.get("verbose", True))
    ...
    if hasattr(result, "coef") and result.coef is not None:
        for i in range(nstudy):
            results_df[f"BETA_EST_{i+1}"] = result.coef[i, :]

    if hasattr(result, "coef_sd") and result.coef_sd is not None:
        for i in range(nstudy):
            results_df[f"BETA_SD_{i+1}"] = result.coef_sd[i, :]
