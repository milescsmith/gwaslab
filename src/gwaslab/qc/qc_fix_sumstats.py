...
from gwaslab.qc.qc_pattern import (
    CHR_PATTERN_EXTRACT,
    CHR_PREFIX_PATTERN,
    FLAGS,
    RSID_PATTERN,
...
def _fix_ID(sumstats_obj: Union["Sumstats", pd.DataFrame],
    ...
    if snpid in sumstats.columns:
        ...
        if is_snpid_string_na.sum() > 0:
            log.log_operation(f"Converting {is_snpid_string_na.sum()} NA strings in SNPID to pd.NA", indent=1, verbose=verbose)
    ...
    if rsid in sumstats.columns:
        ...
        if is_rsid_string_na.sum() > 0:
            log.log_operation(f"Converting {is_rsid_string_na.sum()} NA strings in rsID to pd.NA", indent=1, verbose=verbose)
...
def _strip_SNPID(sumstats_or_dataframe: Union["Sumstats", pd.DataFrame], snpid: str = "SNPID", overwrite: bool = False, verbose: bool = True, log: Log = Log()) -> pd.DataFrame:
    ...
    # check if SNPID is NA
    log.write(f" -Stripping {sum(is_chrposrefalt)} non-NA fixable SNPIDs...",verbose=verbose)
...
def _flip_SNPID(sumstats_or_dataframe: Union["Sumstats", pd.DataFrame], snpid: str = "SNPID", overwrite: bool = False, verbose: bool = True, log: Log = Log()) -> pd.DataFrame:
    ...
    log.write(f" -Flipping {sum(is_chrposrefalt)} non-NA fixable SNPIDs...",verbose=verbose)
...
def _remove_dup(sumstats_obj: Union["Sumstats", pd.DataFrame], mode: str = "dm", chrom: str = "CHR", pos: str = "POS", snpid: str = "SNPID", ea: str = "EA", nea: str = "NEA", rsid: str = "rsID", keep: str = "first", keep_col: str = "P", remove_na: bool = False, keep_ascend: bool = True, verbose: bool = True, log: Log = Log()) -> pd.DataFrame:
    ...
    log.log_operation(f"Removing mode:{mode}", verbose=verbose)
    if keep_col == "P" and ("P" in sumstats.columns):
        zero_count = (sumstats["P"] == 0).sum()
        if zero_count > 1:
            log.log_operation(f"Detected {zero_count} variants with P=0; converting to MLOG10P and sorting descending", verbose=verbose)
    ...
    if keep_col is not None :
        if keep_col in sumstats.columns:
            log.log_operation_start(f"sort the sumstats using {keep_col}", verbose=verbose)
    ...
    if "n" in mode or remove_na==True:
        ...
        after_number=len(sumstats)
        log.log_variants_removed(pre_number - after_number, reason=f"with NA values in {specified_columns}", verbose=verbose)
...
def _convert_sex_chromosomes_to_numeric(sumstats, fixable_indices,
    ...
    for label_lower, numeric_str, label_orig, description in chromosome_mappings:
        if label_lower in sex_chr_values_lower:
            ...
            log.write(f" -Standardizing {description} chromosome notations: {label_orig} to {numeric_str}...", verbose=verbose)
...
def _fix_pos(sumstats_obj: Union["Sumstats", pd.DataFrame], pos: str = "POS", status: str = "STATUS", remove: bool = False, verbose: bool = True, lower_limit: int = 0, upper_limit: int | None = None, limit: int = 250000000, skip_status: bool = False, log: Log = Log()) -> pd.DataFrame:
    """
...
def fastnormalizeallele(insumstats: pd.DataFrame, pos: str = "POS", nea: str = "NEA", ea: str = "EA", status: str = "STATUS", chunk: int = 3000000, log: Log = Log(), verbose: bool = False) -> tuple[pd.DataFrame, np.ndarray]:
    log.write(f" -Number of variants to check:{len(insumstats)}", verbose=verbose)
    log.write(f" -Chunk size:{chunk}", verbose=verbose)
...
def flip_by_inverse(sumstats: pd.DataFrame, matched_index: pd.Series, log: Log, verbose: bool, cols: list[str] | None = None, factor: float = 1) -> pd.DataFrame:
    """
...
def flip_by_subtract(sumstats: pd.DataFrame, matched_index: pd.Series, log: Log, verbose: bool, cols: list[str] | None = None, factor: float = 1) -> pd.DataFrame:
    """
...
def flip_by_sign(sumstats: pd.DataFrame, matched_index: pd.Series, log: Log, verbose: bool, cols: list[str] | None = None) -> pd.DataFrame:
    """
...
def _sort_column(sumstats_obj: Union["Sumstats", pd.DataFrame], verbose: bool = True, log: Log = Log(), order: list[str] | None = None) -> pd.DataFrame:
    """
