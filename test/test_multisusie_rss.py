...
class TestMultiSuSiERSS(unittest.TestCase):
    ...
    def create_summary_statistics(geno_list):
        ...
        y_list = [geno.dot(beta) + rng.standard_normal(geno.shape[0])
                  for (geno, beta) in zip(geno_list, beta_list, strict=False)]
        y_list = [y - np.mean(y) for y in y_list]

        # Calculate summary statistics
        XTY_list = [geno.T.dot(y) for (geno, y) in zip(geno_list, y_list, strict=False)]
        XTX_diagonal_list = [np.diagonal(geno.T.dot(geno)) for geno in geno_list]

        with np.errstate(divide="ignore", invalid="ignore"):
            beta_hat_list = [XTY / XTX_diag
                            for (XTY, XTX_diag) in zip(XTY_list, XTX_diagonal_list, strict=False)]
        ...
        residuals_list = [np.expand_dims(y, 1) - (geno * beta_hat)
                          for (y, geno, beta_hat) in zip(y_list, geno_list, beta_hat_list, strict=False)]
        sum_of_squared_residuals_list = [np.sum(resid ** 2, axis=0)
                                         for resid in residuals_list]
        se_list = [np.sqrt(ssr / ((N - 2) * XTX))
                   for (ssr, N, XTX) in zip(sum_of_squared_residuals_list, N_list, XTX_diagonal_list, strict=False)]
    ...
    def create_sumstats_objects(beta_hat_list, se_list, n_list, snp_meta):
        ...
        for i, (beta_hat, se, n, study_name) in enumerate(zip(beta_hat_list, se_list, n_list, study_names, strict=False)):
    ...
    def test_run_multisusie_rss(self):
        ...
        for snpid, expected_pip in zip(causal_snpids, expected_pips, strict=False):
