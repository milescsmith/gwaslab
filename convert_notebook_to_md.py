...
def extract_image_data(output: dict[str, Any]) -> tuple | None:
...
def parse_html_table(html: str) -> tuple[list[str], list[list[str]]] | None:
...
def html_table_to_markdown(html: str, max_rows: int = 10, text_plain: str | None = None) -> str | None:
...
def convert_notebook_to_markdown(notebook_path: Path, output_path: Path | None = None,
                                 images_dir: Path | None = None) -> Path:
...
def find_notebook_in_examples(notebook_name: str, examples_dir: Path) -> Path | None:
